...
Add a genre dropdown to the story briefing panel so users can target a specific narrative mode before generation.

- Dropdown options: Fantasy, Sci-Fi, Horror, Noir, Romance, Thriller, Historical Fiction, Fairy Tale, and a freeform Custom entry
- Selected genre feeds into `prompt_builder.py` as a first-class calibration parameter alongside the four vibe sliders
- Genre influences tone vocabulary, scene-setting language, and expected narrative structure in the system prompt

### 2 — UI / UX improvements

Continued refinement of the editorial studio interface.

- Chapter-by-chapter progress visualisation in the streaming preview panel
- Genre badge displayed alongside vibe readouts in the history feed
- Outline view: collapsible table of contents with per-chapter status indicators (draft / revised / accepted)
- Mobile-responsive layout for the three-column control surface
- Export actions: copy to clipboard, download as Markdown or plain text

### 4 - PDF generator

Generate PDF with a background design to match the genre.
